...
MaterialData makeDefaultMaterial(const std::string& name = "") {
    MaterialData m;
    m.name = name;
...
bool isTripletArray(const Json& j) {
    return j.isArray() && j.size() == 3 && j[0].isNumber() && j[1].isNumber() && j[2].isNumber();
}

std::array<double, 3> readTriple(const Json& j, const std::string& what) {
    if (j.isArray()) {
        if (j.size() != 3) {
            throw std::runtime_error("Expected 3 values for " + what);
        }
        if (!j[0].isNumber() || !j[1].isNumber() || !j[2].isNumber()) {
            throw std::runtime_error("Expected numeric 3-vector for " + what);
        }
    ...
    if (j.isObject()) {
        ...
        if (has("x", "y", "z")) {
            return {o.at("x").asNumber(), o.at("y").asNumber(), o.at("z").asNumber()};
        }
        if (has("r", "g", "b")) {
            return {o.at("r").asNumber(), o.at("g").asNumber(), o.at("b").asNumber()};
        }
    }
...
ColorData parseColor(const Json& j, const std::string& what = "color") {
    const auto v = readTriple(j, what);
    return ColorData(v[0], v[1], v[2]);
}

Vetor parseVector(const Json& j, const std::string& what = "vector") {
    const auto v = readTriple(j, what);
    return Vetor(v[0], v[1], v[2]);
}

Ponto parsePoint(const Json& j, const std::string& what = "point") {
    const auto v = readTriple(j, what);
...
MaterialData parseMaterialObject(const Json& node, const std::string& fallbackName) {
    if (!node.isObject()) {
    ...
    if (node.contains("ka")) {
        m.ka = parseColor(node["ka"], m.name + ".ka");
    }
    if (node.contains("ks")) {
        m.ks = parseColor(node["ks"], m.name + ".ks");
    }
    if (node.contains("kr")) {
        m.kr = parseColor(node["kr"], m.name + ".kr");
    }
    if (node.contains("kt")) {
        m.kt = parseColor(node["kt"], m.name + ".kt");
    }
...
std::map<std::string, MaterialData> parseMaterials(const Json& materialsNode) {
    if (!materialsNode.isObject()) {
        throw std::runtime_error("'definitions.materials' must be an object");
    }
...
MaterialData resolveMaterial(const Json& node, const std::map<std::string, MaterialData>& table) {
    if (node.isString()) {
    ...
    if (node.isObject()) {
        return parseMaterialObject(
            node,
            node.contains("name") && node["name"].isString() ? node["name"].asString() : "");
    }

    throw std::runtime_error("Invalid material format");
}

CameraData parseCamera(const Json& node) {
    if (!node.isObject()) {
    ...
    if (node.contains("image_width")) {
        cam.image_width = static_cast<int>(requireNumber(node, "image_width"));
    }
    if (node.contains("image_height")) {
        cam.image_height = static_cast<int>(requireNumber(node, "image_height"));
    }
    ...
    if (node.contains("vup")) {
        cam.upVector = parseVector(node["vup"], "camera.vup");
    }

    return cam;
}

LightData parseLight(const Json& node) {
    if (!node.isObject()) {
    ...
    for (const auto& [key, val] : node.asObject()) {
        if (key == "name" || key == "position" || key == "color") {
            continue;
        }
...
TransformData parseTransform(const Json& node) {
    if (!node.isObject()) {
...
std::vector<TransformData> parseTransforms(const Json& node) {
    if (!node.isArray()) {
...
ObjectData parseObject(const Json& node, const std::map<std::string, MaterialData>& materials) {
    if (!node.isObject()) {
    ...
    auto isPositionKey = [](const std::string& key) {
        static const std::vector<std::string> hints = {
            "center", "position", "point", "origin", "point_on_plane", "relativePos"};
    ...
    if (node.contains("relativePos")) {
        obj.relativePos = parsePoint(node["relativePos"], "object.relativePos");
    }
    ...
    for (const auto& [key, val] : node.asObject()) {
        if (key == "type" || key == "material" || key == "transform" || key == "name" || key == "relativePos") {
            continue;
        }
...
SceneData build(const Json& root) {
    if (!root.isObject()) {
...
SceneData SceneJsonLoader::loadFile(const std::string& filename) {
    JsonParser parser;
    const Json root = parser.parseFile(filename);
    return build(root);
}

SceneData SceneJsonLoader::loadString(const std::string& text) {
    JsonParser parser;
    const Json root = parser.parseString(text);
    return build(root);
}
